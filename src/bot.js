...
import { replies } from './responses/replies.js';
...
function getRandomResponse(key) {
    ...
    for (const { reply, weight } of options) {
        if (randomValue <= weight) return reply;

        randomValue -= weight;
    }

    return options[1].reply;
...
client.on('messageCreate', async message => {
    ...
    // Verifica se a mensagem contém "deus" em minúsculo e isolada
    if (/\bdeus\b/.test(message.content)) {
        await message.reply(getRandomResponse('Deus'));
    }

    if (/\broger\b/i.test(message.content) || (message.content.toLowerCase().includes('<@838046045891067936>'))) {
        await message.reply(getRandomResponse('Roger'));
    }
});
